...
public:
    ...
    ImageController(int h, int w, int c): height_(h), width_(w), channel_(c) {
        data_ = std::make_unique<unsigned char[]>(h * w * c);
    }

    template <typename T>
    void write(T value, int y, int x, int c) {
        double normalized_value;
        unsigned char final_value;
        if constexpr (std::is_same_v<T, char> || std::is_same_v<T, int> || std::is_same_v<T, unsigned int>) {
            value = std::clamp(value, 0, 255);
            normalized_value = static_cast<unsigned int>(value) / 255.99;
        ...
        normalized_value = linear_to_gamma(normalized_value);
        final_value = static_cast<unsigned char>(255.99 * normalized_value);

        data_[y * width_ * channel_ + x * channel_ + c] = final_value;
    }

    void write(color value, int y, int x) {
    ...
    void save(const char* filename) {
        ...
        svpng(fp, width_, height_, data_.get(), channel_ == 4);
        fclose(fp);
    }
private:
    std::unique_ptr<unsigned char[]> data_;
    int height_, width_, channel_;
};